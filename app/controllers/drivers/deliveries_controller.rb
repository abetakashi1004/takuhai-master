class Drivers::DeliveriesController < ApplicationController
  def new
  	...
  	@completes = @takeout.where(status: 2)
  	@backs = @takeout.where(status: 3)
  	@yets = @takeout.where(status: 1)
  end
  ...
  def create
  	...
  	current_slip_number = params[:delivery][:package_id]
	  	if Package.where(slip_number: current_slip_number).exists?#その伝票番号が存在するかどうか
		  	...
		  	if Takeout.where(package_id: just_package.id).exists?#持ち出し登録しているかどうか
